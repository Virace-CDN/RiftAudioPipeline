...
| `src/rift_audio_pipeline/pipeline.py` | 4 | 66 |
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`load_baidu_app_credentials`、`resolve_token_store`、`token_store.load_token`、`BaiduCredentials`、`BaiduPanClient`、`strftime`、`datetime.now`、`Path`、`client.get_quota`、`quota.get`、`client.create_directory`、`local_sample.parent.mkdir`
...
- 实现方式：
  ...
  2. 在上下文 `urlopen(request, timeout=30)` 中执行资源操作。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 实现方式：
  ...
  3. 根据条件 `not encoded_content` 执行分支处理。
...
- 返回说明：（Docstring 未提供）
- 可能抛出：
  - RuntimeError（由语法树扫描得到）
- 关键调用：`_http_get_json`、`quote`、`replace`、`payload.get`、`RuntimeError`、`strip`、`decode`、`base64.b64decode`
...
- 作用：构建最新版本对象。
- 实现方式：
  1. 返回 `LatestVersions(game_version=version, game_manifest_url=manifest_url, …` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
| `version` | `str` | `-` | `positional_or_keyword` | （Docstring 未提供） |
...
- 作用：构建历史状态对象。
- 实现方式：
  1. 返回 `LocalRunState(schema_version=1, game_version=version, game_manifest_u…` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
| `version` | `str` | `-` | `positional_or_keyword` | （Docstring 未提供） |
...
- 实现方式：
  ...
  2. 返回 `evaluate_update_need_with_latest(region=TEST_REGION, latest_versions=…` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`_build_latest_versions`、`_build_previous_state`、`evaluate_update_need_with_latest`
...
- 作用：脚本主入口。
- 实现方式：
  1. 遍历 `versions` 执行批量处理。
  2. 遍历 `cases` 执行批量处理。
  3. 执行 `print` 触发副作用逻辑。
  4. 根据条件 `detail_decision is not None` 执行分支处理。
  5. 根据条件 `failed` 执行分支处理。
  6. 返回 `0` 作为结果。
- 参数：
参数：无。
- 返回类型：`int`
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`_fetch_manifest_url_from_repo`、`LiveCase`、`_run_case`、`results.append`、`failed.append`、`json.dumps`、`filter_wad_changes_by_bin_voice_paths`、`LIVE_OUTPUT_DIR.mkdir`、`detail_file.write_text`
...
- 可见性：内部函数
- 源码位置：`scripts/test_pipeline_first_diff_live.py:49`
- 作用：构建命令行参数。
- 实现方式：
...
- 可见性：内部函数
- 源码位置：`scripts/test_pipeline_first_diff_live.py:181`
- 作用：配置全局日志输出等级。
- 实现方式：
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`upper`、`level.strip`、`logger.remove`、`logger.add`

### 函数 `_http_get_json`

#### `_http_get_json(url: str) -> dict[str, Any]`
- 可见性：内部函数
- 源码位置：`scripts/test_pipeline_first_diff_live.py:196`
- 作用：执行 HTTP GET 并解析 JSON。
- 实现方式：
  1. 调用 `Request` 并写入 `request`。
  2. 在上下文 `urlopen(request, timeout=30)` 中执行资源操作。
...
#### `_fetch_manifest_url_from_repo(version: str) -> str`
- 可见性：内部函数
- 源码位置：`scripts/test_pipeline_first_diff_live.py:211`
...
- 实现方式：
  ...
  4. 遍历 `release_items` 执行批量处理。
  5. 通过 `try/except` 包裹核心逻辑并处理异常。
  6. 调用 `replace` 并写入 `encoded_content`。
  7. 根据条件 `not encoded_content` 执行分支处理。
...
- 可能抛出：
  - RuntimeError（由语法树扫描得到）
- 关键调用：`RiotGameData`、`releases.load_game_data`、`get`、`strip`、`item.get`、`version.strip`、`_http_get_json`、`quote`、`Request`、`urlopen`、`decode`、`response.read`

### 函数 `_limit_targets_to_units`

#### `_limit_targets_to_units(targets: object, limit_units: int) -> object`
- 可见性：内部函数
- 源码位置：`scripts/test_pipeline_first_diff_live.py:244`
- 作用：将处理目标截断到指定数量（优先英雄）。
- 实现方式：
  1. 根据条件 `limit_units <= 0` 执行分支处理。
  2. 调用 `tuple` 并写入 `champion_ids`。
  3. 调用 `tuple` 并写入 `map_ids`。
  4. 根据条件 `selected_champions` 执行分支处理。
...
- 可见性：内部函数
- 源码位置：`scripts/test_pipeline_first_diff_live.py:261`
- 作用：构建测试用“最新版本”覆盖对象。
- 实现方式：
  1. 调用 `version.strip` 并写入 `normalized_version`。
  2. 根据条件 `not normalized_version` 执行分支处理。
  3. 调用 `get_latest_versions` 并写入 `latest`。
  4. 调用 `_fetch_manifest_url_from_repo` 并写入 `manifest_url`。
  5. 返回 `LatestVersions(game_version=normalized_version, game_manifest_url=man…` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
| `version` | `str` | `-` | `positional_or_keyword` | （Docstring 未提供） |
...
- 可能抛出：
  - ValueError（由语法树扫描得到）
- 关键调用：`version.strip`、`ValueError`、`get_latest_versions`、`_fetch_manifest_url_from_repo`、`LatestVersions`

### 函数 `_is_champion_manifest_wad_path`

#### `_is_champion_manifest_wad_path(path: str) -> bool`
- 可见性：内部函数
- 源码位置：`scripts/test_pipeline_first_diff_live.py:277`
...
- 可见性：内部函数
- 源码位置：`scripts/test_pipeline_first_diff_live.py:283`
- 作用：构建 diff 联调快捷筛选函数。
- 说明：规则： - 按 `update_paths` 顺序逐个探测； - 优先命中“可解包英雄”并立即返回； - 若无英雄可解包但存在其他可解包条目，返回首个可解包结果； - 若均不可解包，返回聚合后的“全跳过”结果。
- 实现方式：
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`kwargs.get`、`replace`、`path.strip`、`original_filter`、`all_decisions.extend`、`all_skipped.update`、`_is_champion_manifest_wad_path`、`logger.info`、`ManifestVoiceFilterResult`

### 函数 `_patch_pipeline_for_local_live_test`

#### `@contextmanager`
- 可见性：内部函数
- 源码位置：`scripts/test_pipeline_first_diff_live.py:344`
- 作用：临时 patch Pipeline 状态路径与目标解析函数。
- 装饰器：`contextmanager`
- 实现方式：
  1. 根据条件 `diff_smoke_single_champion` 执行分支处理。
  2. 通过 `try/except` 包裹核心逻辑并处理异常。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`original_resolve_processing_targets`、`_limit_targets_to_units`、`original_resolve_all_processing_targets`、`original_evaluate_update_need`、`load_local_state`、`evaluate_update_need_with_latest`、`_build_single_champion_smoke_filter`

### 函数 `_prepare_first_run_state`

#### `_prepare_first_run_state(state_file: Path) -> None`
- 可见性：内部函数
- 源码位置：`scripts/test_pipeline_first_diff_live.py:408`
...
- 可见性：内部函数
- 源码位置：`scripts/test_pipeline_first_diff_live.py:415`
- 作用：准备非首次 diff 状态（写入旧版本历史状态）。
- 实现方式：
  1. 调用 `get_latest_versions` 并写入 `latest`。
  2. 根据条件 `previous_version.strip() == latest.game_version.strip()` 执行分支处理。
...
- 可能抛出：
  - ValueError（由语法树扫描得到）
- 关键调用：`get_latest_versions`、`previous_version.strip`、`latest.game_version.strip`、`ValueError`、`_fetch_manifest_url_from_repo`、`replace`、`isoformat`、`datetime.now`、`LocalRunState`、`save_local_state`
...
- 可见性：内部函数
- 源码位置：`scripts/test_pipeline_first_diff_live.py:438`
- 作用：构建脚本专用 Pipeline 配置。
- 实现方式：
  1. 返回 `PipelineConfig(output_path=output_path, game_region=region, game_path…` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 可见性：内部函数
- 源码位置：`scripts/test_pipeline_first_diff_live.py:482`
- 作用：收集 upload 模式所需压缩包。
- 实现方式：
  1. 根据条件 `game_version is None` 执行分支处理。
  2. 根据条件 `not package_dir.is_dir()` 执行分支处理。
  3. 调用 `tuple` 并写入 `archives`。
  4. 根据条件 `not archives` 执行分支处理。
  5. 根据条件 `limit_units > 0` 执行分支处理。
...
- 返回说明：（Docstring 未提供）
- 可能抛出：
  - FileNotFoundError（由语法树扫描得到）
- 关键调用：`packages_root.is_dir`、`FileNotFoundError`、`packages_root.iterdir`、`item.is_dir`、`item.name.casefold`、`game_version.strip`、`package_dir.is_dir`、`package_dir.rglob`、`item.is_file`、`casefold`、`item.as_posix`

### 函数 `main`

#### `main() -> int`
- 可见性：公开函数
- 源码位置：`scripts/test_pipeline_first_diff_live.py:519`
- 作用：脚本主入口。
- 实现方式：
  1. 调用 `parse_args` 并写入 `args`。
  2. 执行 `_configure_log_level` 触发副作用逻辑。
  3. 调用 `resolve` 并写入 `work_dir`。
  4. 根据条件 `args.clean and work_dir.exists()` 执行分支处理。
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`parse_args`、`_build_parser`、`_configure_log_level`、`resolve`、`expanduser`、`Path`、`work_dir.exists`、`shutil.rmtree`、`work_dir.mkdir`、`load_baidu_app_credentials`、`resolve_token_store`、`token_store.load_token`
...
- 实现方式：
  ...
  2. 根据条件 `not file.exists()` 执行分支处理。
  3. 在上下文 `file.open('rb')` 中执行资源操作。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 实现方式：
  ...
  3. 根据条件 `args.clean and work_dir.exists()` 执行分支处理。
...
- 可能抛出：
  - ValueError（由语法树扫描得到）
- 关键调用：`parse_args`、`_build_parser`、`resolve`、`expanduser`、`Path`、`work_dir.exists`、`shutil.rmtree`、`build_simulated_dir`、`get_latest_versions`、`download_game_content_metadata`、`download_lcu_data_wads`、`run_data_updater`
...
- 返回说明：落地后的目标文件路径。
- 可能抛出：未显式声明。
- 关键调用：`logger.debug`、`PatcherManifest`、`_resolve_manifest_files_by_exact_paths`、`_download_manifest_files`、`stage_runtime_file`
...
- 实现方式：
  ...
  4. 调用 `_download_manifest_files` 并写入 `downloaded`。
  5. 遍历 `downloaded` 执行批量处理。
  6. 返回 `tuple(staged)` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 返回说明：落地后的 WAD 路径集合。
- 可能抛出：未显式声明。
- 关键调用：`logger.debug`、`re.escape`、`PatcherManifest`、`_resolve_manifest_files_by_patterns`、`_download_manifest_files`、`as_posix`、`source.relative_to`、`_to_runtime_relative_path`、`staged.append`、`stage_runtime_file`
...
- 说明：运行时路径通常来自 `data.msgpack` 的 `wad.root` / `wad.<region>` 字段， 例如 `Game/DATA/FINAL/Champions/Annie.zh_CN.wad.client`。
- 实现方式：
  1. 执行 `logger.debug` 触发副作用逻辑。
...
- 返回说明：落地后的 WAD 路径集合。
- 可能抛出：未显式声明。
- 关键调用：`logger.debug`、`_to_game_manifest_path`、`PatcherManifest`、`_resolve_manifest_files_by_exact_paths`、`_download_manifest_files`、`as_posix`、`source.relative_to`、`staged.append`、`stage_runtime_file`
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`files.extend`、`manifest.filter_files`、`_dedupe_manifest_files`
...
- 实现方式：
  1. 遍历 `exact_paths` 执行批量处理。
  2. 根据条件 `missing` 执行分支处理。
...
- 关键调用：`casefold`、`replace`、`manifest.files.values`、`raw_path.strip`、`index.get`、`normalized.casefold`、`missing.append`、`selected.append`、`FileNotFoundError`、`_dedupe_manifest_files`
...
- 实现方式：
  ...
  6. 根据条件 `failed` 执行分支处理。
  7. 返回 `tuple(sorted((download_dir / str(file_obj.name).replace('\\', '/') fo…` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
| `manifest` | `PatcherManifest` | `-` | `positional_or_keyword` | （Docstring 未提供） |
...
- 可能抛出：
  - RuntimeError（由语法树扫描得到）
- 关键调用：`download_dir.mkdir`、`time.perf_counter`、`logger.debug`、`asyncio.run`、`manifest.download_files_concurrently`、`RuntimeError`、`replace`、`casefold`、`path.as_posix`
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`replace`、`name.casefold`、`seen.add`、`deduped.append`
...
- 实现方式：
  ...
  2. 根据条件 `normalized.startswith('Game/')` 执行分支处理。
  3. 根据条件 `normalized.startswith('DATA/')` 执行分支处理。
  4. 在错误场景抛出 `ValueError(f'无法映射到 GAME manifest 路径：{runtime_path}')`。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 可能抛出：
  - ValueError（由语法树扫描得到）
- 关键调用：`replace`、`runtime_path.strip`、`normalized.startswith`、`normalized.removeprefix`、`ValueError`
...
- 实现方式：
  1. 调用 `replace` 并写入 `normalized`。
  2. 根据条件 `normalized == 'content-metadata.json'` 执行分支处理。
  3. 根据条件 `normalized.startswith('DATA/')` 执行分支处理。
  4. 根据条件 `normalized.startswith('Plugins/')` 执行分支处理。
  5. 在错误场景抛出 `ValueError(f'无法映射到运行时目录路径：{manifest_path}')`。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 可能抛出：
  - ValueError（由语法树扫描得到）
- 关键调用：`replace`、`manifest_path.strip`、`normalized.startswith`、`ValueError`
...
- 实现方式：
  ...
  4. 根据条件 `not isinstance(data_file_base, Path)` 执行分支处理。
...
- 可能抛出：
  - RuntimeError（由语法树扫描得到）
- 关键调用：`_initialize_unpack_config`、`DataUpdater`、`updater.check_and_update`、`RuntimeError`、`logger.info`
...
- 实现方式：
  ...
  4. 遍历 `champions_raw.items()` 执行批量处理。
  5. 调用 `set` 并写入 `normalized_map_ids`。
  6. 遍历 `map_ids` 执行批量处理。
  7. 返回 `AudioProcessingTargets(champion_ids=tuple(sorted(champion_ids)), map_…` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 可能抛出：未显式声明。
- 关键调用：`read_data`、`payload.get`、`item.casefold`、`champions_raw.items`、`casefold`、`champion_data.get`、`champion_ids.add`、`logger.warning`、`normalized_map_ids.add`、`AudioProcessingTargets`
...
- 实现方式：
  ...
  6. 返回 `AudioProcessingTargets(champion_ids=tuple(champion_ids), map_ids=tupl…` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
| `data_file_base` | `Path` | `-` | `positional_or_keyword` | `data` 文件基础路径（不带后缀）。 |
...
- 返回说明：全量目标 ID 集合。
- 可能抛出：未显式声明。
- 关键调用：`read_data`、`payload.get`、`isdigit`、`AudioProcessingTargets`
...
- 说明：规则： - 当 `champion_ids` 与 `map_ids` 同时为空时，按全量模式返回全部英雄与地图路径。 - 否则仅返回指定 ID 的路径。 - 路径统一转换为运行时格式（以 `Game/` 开头）。
- 实现方式：
  1. 调用 `read_data` 并写入 `payload`。
  2. 调用 `payload.get` 并写入 `champions_raw`。
  3. 调用 `payload.get` 并写入 `maps_raw`。
  4. 根据条件 `isinstance(champions_raw, Mapping)` 执行分支处理。
  5. 根据条件 `isinstance(maps_raw, Mapping)` 执行分支处理。
...
- 可能抛出：未显式声明。
- 关键调用：`read_data`、`payload.get`、`champions_raw.items`、`_extract_runtime_wad_paths_from_payload`、`runtime_paths.setdefault`、`path.casefold`、`maps_raw.items`、`runtime_paths.values`
...
- 实现方式：
  ...
  2. 根据条件 `champion_ids or map_ids` 执行分支处理。
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`BinUpdater`、`updater.update`、`logger.info`
...
- 实现方式：
  ...
  3. 根据条件 `champion_ids or map_ids` 执行分支处理。
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`_reset_singleton_instance`、`DataReader`、`unpack_champions`、`unpack_maps`、`logger.info`、`unpack_audio_all`、`reader.write_unknown_categories_to_file`
...
- 实现方式：
  1. 根据条件 `not isinstance(payload_item, Mapping)` 执行分支处理。
  2. 调用 `payload_item.get` 并写入 `wad_raw`。
  3. 根据条件 `not isinstance(wad_raw, Mapping)` 执行分支处理。
  4. 根据条件 `include_root_wad` 执行分支处理。
  5. 遍历 `candidates` 执行批量处理。
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`payload_item.get`、`wad_raw.get`、`candidates.insert`、`_normalize_runtime_wad_path`、`normalized.setdefault`、`path.casefold`、`normalized.values`
...
- 实现方式：
  1. 调用 `replace` 并写入 `normalized`。
  2. 根据条件 `not normalized` 执行分支处理。
  3. 根据条件 `normalized.startswith('Game/')` 执行分支处理。
  4. 根据条件 `normalized.startswith('DATA/')` 执行分支处理。
  5. 返回 `None` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`replace`、`path.strip`、`normalized.startswith`
...
- 实现方式：
  ...
  6. 执行 `auth_url_parser.set_defaults` 触发副作用逻辑。
  7. 调用 `subparsers.add_parser` 并写入 `exchange_parser`。
  8. 执行 `exchange_parser.add_argument` 触发副作用逻辑。
- 参数：
参数：无。
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`build_parser`、`parser.parse_args`、`handler`
...
- 实现方式：
  ...
  2. 在上下文 `BaiduOAuthClient()` 中执行资源操作。
  3. 执行 `print` 触发副作用逻辑。
  4. 返回 `0` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`load_baidu_app_credentials`、`Path`、`BaiduOAuthClient`、`client.build_authorization_url`
...
- 实现方式：
  1. 调用 `load_baidu_app_credentials` 并写入 `credentials`。
  2. 调用 `resolve_token_store` 并写入 `token_store`。
  3. 在上下文 `BaiduOAuthClient()` 中执行资源操作。
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`load_baidu_app_credentials`、`Path`、`resolve_token_store`、`BaiduOAuthClient`、`client.exchange_code_for_token`、`token_store.save_token`、`mask_secret`
...
- 实现方式：
  1. 调用 `load_baidu_app_credentials` 并写入 `credentials`。
  2. 调用 `resolve_token_store` 并写入 `token_store`。
  3. 根据条件 `refresh_token is None` 执行分支处理。
  4. 在上下文 `BaiduOAuthClient()` 中执行资源操作。
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`load_baidu_app_credentials`、`Path`、`resolve_token_store`、`token_store.load_token`、`BaiduOAuthClient`、`client.refresh_access_token`、`token_store.save_token`、`mask_secret`
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`resolve_token_store`、`Path`、`token_store.load_token`、`mask_secret`
...
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
| `self` | `-` | `-` | `positional_or_keyword` | （Docstring 未提供） |
...
- 实现方式：
  ...
  5. 根据条件 `access_token is None` 执行分支处理。
  6. 根据条件 `refresh_token is None` 执行分支处理。
  7. 根据条件 `not isinstance(expires_in_value, int)` 执行分支处理。
  8. 返回 `cls(access_token=access_token, refresh_token=refresh_token, expires_i…` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
| `cls` | `-` | `-` | `positional_or_keyword` | （Docstring 未提供） |
...
- 关键调用：`_as_non_empty_str`、`payload.get`、`BaiduOAuthError`、`cls`、`_to_utc_iso`、`datetime.now`
...
- 装饰器：`classmethod`
- 实现方式：
  1. 返回 `cls(access_token=str(data['access_token']), refresh_token=str(data['r…` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
| `cls` | `-` | `-` | `positional_or_keyword` | （Docstring 未提供） |
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`cls`、`data.get`、`_as_non_empty_str`
...
- 作用：读取本地 token。
- 实现方式：
  1. 根据条件 `not self._token_file.exists()` 执行分支处理。
  2. 调用 `self._token_file.read_text` 并写入 `raw_text`。
  3. 调用 `json.loads` 并写入 `data`。
  4. 根据条件 `not isinstance(data, dict)` 执行分支处理。
...
- 关键调用：`self._token_file.exists`、`FileNotFoundError`、`self._token_file.read_text`、`json.loads`、`ValueError`、`BaiduOAuthToken.from_dict`
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`self._token_file.parent.mkdir`、`json.dumps`、`token.to_dict`、`self._token_file.write_text`、`os.chmod`
...
- 实现方式：
  ...
  2. 调用 `load_openapi_client_module` 并写入 `self._openapi_client`。
  3. 调用 `self._openapi_client.ApiClient` 并写入 `self._api_client`。
  4. 调用 `importlib.import_module` 并写入 `auth_api_module`。
  5. 调用 `importlib.import_module` 并写入 `exception_module`。
  6. 调用 `auth_api_module.AuthApi` 并写入 `self._auth_api`。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
| `self` | `-` | `-` | `positional_or_keyword` | （Docstring 未提供） |
...
- 作用：生成授权码模式 URL。
- 实现方式：
  1. 根据条件 `state is not None` 执行分支处理。
...
- 实现方式：
  ...
  2. 返回 `BaiduOAuthToken.from_payload(payload=payload, fallback_refresh_token=…` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
| `self` | `-` | `-` | `positional_or_keyword` | （Docstring 未提供） |
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`self._invoke_auth_api`、`self._auth_api.oauth_token_code2token`、`BaiduOAuthToken.from_payload`
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`self._invoke_auth_api`、`self._auth_api.oauth_token_refresh_token`、`BaiduOAuthToken.from_payload`
...
- 返回说明：（Docstring 未提供）
- 可能抛出：
  - BaiduOAuthError（由语法树扫描得到）
- 关键调用：`action`、`BaiduOAuthError`、`_extract_api_exception_message`、`_payload_to_dict`、`_as_non_empty_str`、`payload.get`
...
- 实现方式：
  ...
  2. 根据条件 `resolved_app_key is None` 执行分支处理。
  3. 根据条件 `resolved_secret_key is None` 执行分支处理。
  4. 返回 `BaiduAppCredentials(app_key=resolved_app_key, secret_key=resolved_sec…` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 可能抛出：
  - ValueError（由语法树扫描得到）
- 关键调用：`_as_non_empty_str`、`os.getenv`、`_load_kv_file`、`file_values.get`、`ValueError`、`BaiduAppCredentials`
...
- 实现方式：
  1. 根据条件 `not path.exists()` 执行分支处理。
  2. 遍历 `path.read_text(encoding='utf-8').splitlines()` 执行批量处理。
  3. 返回 `result` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`path.exists`、`splitlines`、`path.read_text`、`line.strip`、`stripped.startswith`、`stripped.split`、`key.strip`、`_normalize_config_value`、`raw_value.strip`
...
- 作用：将 SDK 响应统一转换为字典。
- 实现方式：
  1. 根据条件 `isinstance(payload, dict)` 执行分支处理。
  2. 调用 `getattr` 并写入 `to_dict`。
  3. 根据条件 `callable(to_dict)` 执行分支处理。
  4. 调用 `getattr` 并写入 `attribute_map`。
  5. 根据条件 `isinstance(attribute_map, dict)` 执行分支处理。
  6. 根据条件 `isinstance(payload, Mapping)` 执行分支处理。
  7. 在错误场景抛出 `BaiduOAuthError(message=f'无法解析 OAuth SDK 响应类型：{type(payload)!r}')`。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 可能抛出：
  - BaiduOAuthError（由语法树扫描得到）
- 关键调用：`callable`、`to_dict`、`BaiduOAuthError`
...
- 作用：将对象安全转换为非空字符串。
- 实现方式：
  1. 根据条件 `isinstance(value, str)` 执行分支处理。
  2. 返回 `None` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 作用：标准化配置值，去除包裹引号。
- 实现方式：
  1. 根据条件 `len(value) >= 2 and value[0] == value[-1] and (value[0] in {"'", '"'})` 执行分支处理。
  2. 返回 `value` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 实现方式：
  1. 返回 `value.astimezone(timezone.utc).isoformat(timespec='seconds').replace(…` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 作用：初始化客户端。
- 实现方式：
  1. 调用 `_normalize_remote_path` 并写入 `self._remote_dir`。
  2. 调用 `credentials.refresh_token.strip` 并写入 `self._refresh_token`。
  3. 根据条件 `token_store is not None` 执行分支处理。
  4. 根据条件 `not self._refresh_token` 执行分支处理。
  5. 执行 `ensure_official_sdk_path` 触发副作用逻辑。
  6. 调用 `load_openapi_client_module` 并写入 `self._openapi_client`。
  7. 调用 `self._openapi_client.ApiClient` 并写入 `self._api_client`。
...
- 关键调用：`_normalize_remote_path`、`credentials.refresh_token.strip`、`token_store.load_token`、`ValueError`、`ensure_official_sdk_path`、`load_openapi_client_module`、`self._openapi_client.ApiClient`、`importlib.import_module`、`fileinfo_api_module.FileinfoApi`、`filemanager_api_module.FilemanagerApi`、`fileupload_api_module.FileuploadApi`、`multimedia_api_module.MultimediafileApi`
...
- 实现方式：
  ...
  2. 在上下文 `BaiduOAuthClient(timeout=30.0)` 中执行资源操作。
  3. 根据条件 `self._token_store is not None` 执行分支处理。
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`self._ensure_access_token`、`self._invoke_sdk`、`self._userinfo_api.apiquota`、`_as_int`、`payload.get`、`payload.setdefault`
...
- 实现方式：
  ...
  3. 返回 `self._invoke_sdk(lambda: self._fileinfo_api.xpanfilelist(access_token…` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
| `self` | `-` | `-` | `positional_or_keyword` | （Docstring 未提供） |
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`self._ensure_access_token`、`self._resolve_workdir_path`、`self._invoke_sdk`、`self._fileinfo_api.xpanfilelist`
...
- 实现方式：
  1. 调用 `self._ensure_access_token` 并写入 `access_token`。
  2. 调用 `self._resolve_workdir_path` 并写入 `normalized_dir`。
  3. 返回 `self._invoke_sdk(lambda: self._fileupload_api.xpanfilecreate(access_t…` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
| `self` | `-` | `-` | `positional_or_keyword` | （Docstring 未提供） |
...
- 作用：重命名文件或目录。
- 实现方式：
  1. 返回 `self._filemanager_operation(opera='rename', filelist=payload, ondup=o…` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
| `self` | `-` | `-` | `positional_or_keyword` | （Docstring 未提供） |
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`self._resolve_workdir_path`、`self._filemanager_operation`
...
- 作用：复制文件或目录。
- 实现方式：
  1. 根据条件 `new_name is not None` 执行分支处理。
  2. 返回 `self._filemanager_operation(opera='copy', filelist=[file_payload], on…` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
| `self` | `-` | `-` | `positional_or_keyword` | （Docstring 未提供） |
...
- 实现方式：
  1. 根据条件 `new_name is not None` 执行分支处理。
  2. 返回 `self._filemanager_operation(opera='move', filelist=[file_payload], on…` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
| `self` | `-` | `-` | `positional_or_keyword` | （Docstring 未提供） |
...
- 作用：批量删除路径。
- 实现方式：
  1. 返回 `self._filemanager_operation(opera='delete', filelist=filelist, ondup=…` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
| `self` | `-` | `-` | `positional_or_keyword` | （Docstring 未提供） |
...
- 实现方式：
  1. 根据条件 `not local_path.exists()` 执行分支处理。
  2. 根据条件 `not local_path.is_file()` 执行分支处理。
...
- 返回说明：（Docstring 未提供）
- 可能抛出：
  - BaiduPanApiError（由语法树扫描得到）
  - FileNotFoundError（由语法树扫描得到）
  - ValueError（由语法树扫描得到）
- 关键调用：`local_path.exists`、`FileNotFoundError`、`local_path.is_file`、`ValueError`、`self._ensure_access_token`、`self._resolve_workdir_path`、`local_path.stat`、`_calculate_block_md5s`、`json.dumps`、`self._invoke_sdk`、`self._fileupload_api.xpanfileprecreate`、`_as_non_empty_str`
...
- 实现方式：
  ...
  2. 根据条件 `_as_int(entry.get('isdir')) == 1` 执行分支处理。
  3. 调用 `_as_int` 并写入 `fs_id`。
  4. 根据条件 `fs_id is None` 执行分支处理。
  ...
  8. 根据条件 `not isinstance(meta_list, list) or not meta_list` 执行分支处理。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
| `self` | `-` | `-` | `positional_or_keyword` | （Docstring 未提供） |
...
- 可能抛出：
  - BaiduPanApiError（由语法树扫描得到）
  - IsADirectoryError（由语法树扫描得到）
- 关键调用：`self.get_path_entry`、`_as_int`、`entry.get`、`IsADirectoryError`、`BaiduPanApiError`、`self._ensure_access_token`、`self._invoke_sdk`、`self._multimedia_api.xpanmultimediafilemetas`、`json.dumps`、`metas_payload.get`、`merged.update`
...
- 实现方式：
  ...
  6. 遍历 `range(1, max_attempts + 1)` 执行批量处理。
...
- 可能抛出：
  - BaiduPanApiError（由语法树扫描得到）
- 关键调用：`self._resolve_workdir_path`、`self.get_file_version_info`、`self._ensure_access_token`、`local_path.parent.mkdir`、`local_path.with_suffix`、`self._invoke_sdk_call_api`、`temp_path.open`、`raw_response.read`、`file_obj.write`、`temp_path.replace`、`temp_path.exists`、`temp_path.unlink`
...
- 实现方式：
  1. 调用 `self._resolve_workdir_path` 并写入 `normalized_path`。
  2. 根据条件 `normalized_path == '/'` 执行分支处理。
  3. 根据条件 `normalized_path == self._remote_dir` 执行分支处理。
  4. 调用 `PurePosixPath` 并写入 `path_obj`。
  5. 通过 `try/except` 包裹核心逻辑并处理异常。
  6. 遍历 `listing.get('list', [])` 执行批量处理。
  7. 在错误场景抛出 `FileNotFoundError(f'网盘路径不存在：{normalized_path}')`。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
| `self` | `-` | `-` | `positional_or_keyword` | （Docstring 未提供） |
...
- 可能抛出：
  - FileNotFoundError（由语法树扫描得到）
- 关键调用：`self._resolve_workdir_path`、`PurePosixPath`、`self.list_files`、`FileNotFoundError`、`listing.get`、`entry.get`
...
- 实现方式：
  ...
  3. 根据条件 `opera == 'copy'` 执行分支处理。
  4. 根据条件 `opera == 'move'` 执行分支处理。
  5. 根据条件 `opera == 'rename'` 执行分支处理。
  6. 根据条件 `opera == 'delete'` 执行分支处理。
  7. 在错误场景抛出 `ValueError(f'不支持的 filemanager 操作：{opera}')`。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
| `self` | `-` | `-` | `positional_or_keyword` | （Docstring 未提供） |
...
- 可能抛出：
  - ValueError（由语法树扫描得到）
- 关键调用：`self._ensure_access_token`、`json.dumps`、`self._invoke_sdk`、`self._filemanager_api.filemanagercopy`、`self._filemanager_api.filemanagermove`、`self._filemanager_api.filemanagerrename`、`self._filemanager_api.filemanagerdelete`、`ValueError`
...
- 实现方式：
  ...
  4. 根据条件 `errno not in (None, 0)` 执行分支处理。
  5. 返回 `payload` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 可能抛出：
  - BaiduPanApiError（由语法树扫描得到）
- 关键调用：`action`、`BaiduPanApiError`、`_extract_api_exception_message`、`_payload_to_dict`、`_as_int`、`payload.get`
...
- 实现方式：
  1. 根据条件 `self._oauth_token is not None and _token_not_expired(self._oauth_toke…` 执行分支处理。
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`_token_not_expired`、`self.refresh_access_token`
...
- 实现方式：
  ...
  2. 根据条件 `self._is_under_remote_dir(normalized_path)` 执行分支处理。
  3. 根据条件 `allow_outside` 执行分支处理。
  4. 在错误场景抛出 `ValueError(message)`。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
| `self` | `-` | `-` | `positional_or_keyword` | （Docstring 未提供） |
...
- 关键调用：`self._resolve_remote_path`、`self._is_under_remote_dir`、`warnings.warn`、`ValueError`
...
- 实现方式：
  ...
  2. 根据条件 `not stripped` 执行分支处理。
  3. 根据条件 `stripped.startswith('/')` 执行分支处理。
  4. 根据条件 `self._remote_dir == '/'` 执行分支处理。
...
- 作用：判断路径是否落在工作目录内。
- 实现方式：
  1. 根据条件 `self._remote_dir == '/'` 执行分支处理。
...
- 实现方式：
  ...
  5. 根据条件 `isinstance(attribute_map, dict)` 执行分支处理。
  6. 根据条件 `isinstance(payload, Mapping)` 执行分支处理。
  7. 在错误场景抛出 `BaiduPanApiError(message=f'无法解析 SDK 响应类型：{type(payload)!r}')`。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
| `payload` | `Any` | `-` | `positional_or_keyword` | （Docstring 未提供） |
...
- 可能抛出：
  - BaiduPanApiError（由语法树扫描得到）
- 关键调用：`callable`、`to_dict`、`BaiduPanApiError`
...
- 实现方式：
  ...
  2. 根据条件 `not stripped` 执行分支处理。
  3. 根据条件 `not stripped.startswith('/')` 执行分支处理。
  4. 调用 `posixpath.normpath` 并写入 `normalized`。
  5. 根据条件 `normalized == '.'` 执行分支处理。
  6. 根据条件 `not normalized.startswith('/')` 执行分支处理。
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`path.strip`、`stripped.startswith`、`posixpath.normpath`、`normalized.startswith`
...
- 作用：计算文件分片 MD5 列表。
- 实现方式：
  1. 在上下文 `local_path.open('rb')` 中执行资源操作。
  2. 根据条件 `not block_md5s` 执行分支处理。
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`local_path.open`、`file_obj.read`、`block_md5s.append`、`hexdigest`、`hashlib.md5`
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`datetime.fromisoformat`、`token.expires_at.replace`、`datetime.now`、`expires_at.timestamp`、`now.timestamp`
...
- 作用：将对象安全转换为整数。
- 实现方式：
  1. 根据条件 `isinstance(value, bool)` 执行分支处理。
  2. 根据条件 `isinstance(value, int)` 执行分支处理。
  3. 根据条件 `isinstance(value, str)` 执行分支处理。
  4. 返回 `None` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 实现方式：
  1. 根据条件 `not OFFICIAL_BAIDU_SDK_DIR.exists()` 执行分支处理。
  2. 调用 `str` 并写入 `sdk_path_text`。
  3. 根据条件 `sdk_path_text not in sys.path` 执行分支处理。
...
- 关键调用：`OFFICIAL_BAIDU_SDK_DIR.exists`、`FileNotFoundError`、`sys.path.insert`
...
- 实现方式：
  1. 根据条件 `enable_local_bin and written_files` 执行分支处理。
...
- 可能抛出：未显式声明。
- 关键调用：`write_to_bin_input`、`bin_payloads.items`、`casefold`、`create_local_bin_flag`
...
- 说明：目录相对结构会原样保留。例如 `source_dir/data/maps/map11/map11.bin` 会落地到 `version_dir/bin_input/data/maps/map11/map11.bin`。
- 实现方式：
  1. 根据条件 `not source_dir.is_dir()` 执行分支处理。
  2. 遍历 `sorted(source_dir.rglob('*'), key=lambda path: path.as_posix().casefo…` 执行批量处理。
  3. 返回 `write_many_to_bin_input(version_dir=version_dir, bin_payloads=payload…` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
| `version_dir` | `Path` | `-` | `positional_or_keyword` | 版本目录路径。 |
...
- 关键调用：`source_dir.is_dir`、`FileNotFoundError`、`source_dir.rglob`、`casefold`、`path.as_posix`、`item.is_file`、`item.suffix.casefold`、`as_posix`、`item.relative_to`、`item.read_bytes`、`write_many_to_bin_input`
...
- 实现方式：
  ...
  2. 根据条件 `not raw` 执行分支处理。
  3. 调用 `Path` 并写入 `path`。
  4. 根据条件 `path.is_absolute()` 执行分支处理。
  5. 调用 `Path` 并写入 `normalized`。
  6. 根据条件 `not normalized.parts` 执行分支处理。
  7. 根据条件 `any((part == '..' for part in normalized.parts))` 执行分支处理。
...
- 关键调用：`replace`、`relative_path.strip`、`ValueError`、`Path`、`path.is_absolute`
...
- 实现方式：
  1. 在错误场景抛出 `NotImplementedError(f'待实现：英雄 {alias} 的在线 bin 提取。')`。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 实现方式：
  1. 在错误场景抛出 `NotImplementedError(f'待实现：地图 {map_id} 的在线 bin 提取。')`。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 返回说明：完整的流水线配置对象。
- 可能抛出：未显式声明。
- 关键调用：`os.getenv`、`resolve`、`expanduser`、`Path`、`cls`
...
- 实现方式：
  1. 遍历 `(champions_dir, maps_dir, lcu_dir)` 执行批量处理。
  2. 返回 `base_dir` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 说明：`lol_audio_unpack` 的 `DataUpdater/BinUpdater` 会从该文件读取版本号， 因此在线模式最小游戏目录必须包含该文件。
- 实现方式：
...
- 实现方式：
  1. 返回 `stage_runtime_file(game_path=game_path, relative_path=relative_wad_pa…` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
| `game_path` | `Path` | `-` | `positional_or_keyword` | 游戏根目录路径。 |
...
- 实现方式：
  1. 根据条件 `not source_file.is_file()` 执行分支处理。
  2. 调用 `_normalize_relative_game_path` 并写入 `normalized_path`。
  3. 执行 `target_file.parent.mkdir` 触发副作用逻辑。
  4. 根据条件 `source_file.resolve(strict=False) == target_file.resolve(strict=False)` 执行分支处理。
  5. 根据条件 `target_file.exists()` 执行分支处理。
  6. 根据条件 `strategy == 'hardlink'` 执行分支处理。
...
- 可能抛出：
  - FileNotFoundError：源文件不存在时抛出。
  - ValueError：目标相对路径不合法时抛出。
- 关键调用：`source_file.is_file`、`FileNotFoundError`、`_normalize_relative_game_path`、`target_file.parent.mkdir`、`source_file.resolve`、`target_file.resolve`、`target_file.exists`、`target_file.unlink`、`target_file.hardlink_to`、`shutil.copy2`
...
- 说明：会删除： - `LeagueClient/Plugins/rcp-be-lol-game-data/*.wad` - `manifest/<version>/bin_input/` 目录 - `manifest/<version>/.use_local_bin` 标志
- 实现方式：
  1. 根据条件 `lcu_dir.is_dir()` 执行分支处理。
  2. 根据条件 `bin_input_dir.exists()` 执行分支处理。
  3. 根据条件 `local_bin_flag.exists()` 执行分支处理。
...
- 可能抛出：未显式声明。
- 关键调用：`lcu_dir.is_dir`、`lcu_dir.glob`、`wad_file.is_file`、`wad_file.unlink`、`bin_input_dir.exists`、`bin_input_dir.rglob`、`file_path.is_file`、`shutil.rmtree`、`local_bin_flag.exists`、`local_bin_flag.unlink`
...
- 说明：清理范围： - `default-assets*.wad` - `{region}-assets.wad`
- 实现方式：
  1. 根据条件 `not lcu_dir.is_dir()` 执行分支处理。
  2. 遍历 `patterns` 执行批量处理。
  3. 遍历 `sorted(targets.values(), key=lambda item: item.as_posix().casefold())` 执行批量处理。
...
- 返回说明：已删除文件数。
- 可能抛出：未显式声明。
- 关键调用：`lcu_dir.is_dir`、`lcu_dir.glob`、`file_path.is_file`、`casefold`、`file_path.as_posix`、`targets.values`、`item.as_posix`、`file_path.unlink`
...
- 装饰器：`classmethod`
- 实现方式：
  1. 根据条件 `missing` 执行分支处理。
  2. 返回 `cls(schema_version=int(data['schema_version']), game_version=str(data…` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
| `cls` | `-` | `-` | `positional_or_keyword` | （Docstring 未提供） |
...
- 装饰器：`classmethod`
- 实现方式：
  1. 返回 `cls(path=str(data.get('path', '')), status=str(data.get('status', 'mi…` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
| `cls` | `-` | `-` | `positional_or_keyword` | （Docstring 未提供） |
...
- 实现方式：
  ...
  3. 返回 `cls(region_wad_path=str(data.get('region_wad_path', '')), root_wad_pa…` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
| `cls` | `-` | `-` | `positional_or_keyword` | （Docstring 未提供） |
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`data.get`、`VoicePathStatus.from_dict`、`cls`
...
- 作用：序列化为字典。
- 实现方式：
  1. 返回 `{'region_wad_path': self.region_wad_path, 'root_wad_path': self.root_…` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
| `self` | `-` | `-` | `positional_or_keyword` | （Docstring 未提供） |
...
- 实现方式：
  ...
  3. 返回 `cls(unpack_paths=tuple((str(item) for item in data.get('unpack_paths'…` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
| `cls` | `-` | `-` | `positional_or_keyword` | （Docstring 未提供） |
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`data.get`、`WadVoiceFilterDecision.from_dict`、`cls`
...
- 作用：序列化为字典。
- 实现方式：
  1. 返回 `{'unpack_paths': list(self.unpack_paths), 'skipped_paths': list(self.…` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
| `self` | `-` | `-` | `positional_or_keyword` | （Docstring 未提供） |
...
- 实现方式：
  ...
  6. 根据条件 `latest_game is None` 执行分支处理。
  7. 根据条件 `latest_lcu is None` 执行分支处理。
  8. 返回 `LatestVersions(game_version=latest_game['version'], game_manifest_url…` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 关键调用：`RiotGameData`、`game_data.load_lcu_data`、`game_data.load_game_data`、`game_data.latest_game`、`game_data.latest_lcu`、`RuntimeError`、`LatestVersions`
...
- 实现方式：
  ...
  2. 返回 `ManifestWadChanges(added_paths=tuple(sorted(added_paths, key=str.case…` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 可能抛出：未显式声明。
- 关键调用：`diff_manifests`、`_extract_diff_entry_path`、`ManifestWadChanges`
...
- 实现方式：
  ...
  2. 返回 `_extract_changed_entities_from_paths(paths=set(wad_changes.added_path…` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
| `old_manifest_url` | `str` | `-` | `positional_or_keyword` | 旧版本 manifest URL。 |
...
- 返回说明：变更实体集合。
- 可能抛出：未显式声明。
- 关键调用：`get_manifest_wad_changes`、`_extract_changed_entities_from_paths`
...
- 返回说明：变更实体集合。
- 可能抛出：未显式声明。
- 关键调用：`_normalize_manifest_path`、`path.strip`、`_extract_changed_entities_from_paths`
...
- 说明：规则： - 跳过 `Localized` 路径。 - 不做路径字符串拼接推导，语音资源路径一律来自 BIN 解析。 - 仅当 `*_vo_audio.bnk` 或 `*_vo_audio.wpk` 发生变化时判定为“需要解包”。 - 仅 `*_vo_events.bnk` 变化时标记为“可跳过资源解包”。
- 实现方式：
  1. 根据条件 `max_champion_skin_bin_index < 0` 执行分支处理。
  2. 根据条件 `unit_max_workers < 1` 执行分支处理。
  3. 根据条件 `extractor_prefetch_chunk_concurrency < 1` 执行分支处理。
  4. 调用 `_normalize_update_paths` 并写入 `normalized_paths`。
  5. 根据条件 `not normalized_paths` 执行分支处理。
...
- 关键调用：`ValueError`、`_normalize_update_paths`、`ManifestVoiceFilterResult`、`PatcherManifest`、`_build_manifest_file_index`、`_prepare_voice_filter_bin_output_dir`、`bin_payloads.items`、`casefold`、`_normalize_manifest_path`、`normalized_path.casefold`、`_write_voice_filter_bin_output_file`、`written_bin_keys.add`
...
- 作用：准备 BIN 输出目录。
- 实现方式：
  1. 根据条件 `bin_output_dir.exists()` 执行分支处理。
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`bin_output_dir.exists`、`shutil.rmtree`、`bin_output_dir.mkdir`
...
- 关键调用：`_normalize_manifest_path`、`ValueError`、`Path`、`path.is_absolute`
...
- 实现方式：
  ...
  2. 根据条件 `LOCALIZED_WAD_SEGMENT in normalized_wad_path.casefold()` 执行分支处理。
  3. 调用 `_resolve_root_wad_path` 并写入 `root_wad_path`。
  4. 根据条件 `root_wad_path is None` 执行分支处理。
  5. 调用 `_build_candidate_bin_paths` 并写入 `(entity_type, candidate_bin_paths)`。
  6. 根据条件 `not candidate_bin_paths` 执行分支处理。
  7. 根据条件 `root_wad_path.casefold() not in new_file_index` 执行分支处理。
...
- 返回说明：单个 WAD 的筛选判定结果。
- 可能抛出：未显式声明。
- 关键调用：`_normalize_manifest_path`、`normalized_wad_path.casefold`、`WadVoiceFilterDecision`、`_resolve_root_wad_path`、`_build_candidate_bin_paths`、`root_wad_path.casefold`、`_extract_existing_bin_raws`、`_collect_voice_paths_from_bin_raws`、`bin_raws.keys`、`_collect_voice_path_statuses`、`on_bin_payloads`
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`path.strip`、`diff_manifests`、`diff_wad_headers`、`_build_wad_focus_status_index`、`_load_wad_header`、`statuses.append`、`VoicePathStatus`、`_resolve_inner_path_status`、`item.path.casefold`
...
- 实现方式：
  ...
  2. 根据条件 `file_obj is None` 执行分支处理。
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`file_index.get`、`wad_path.casefold`、`extractor.get_wad_header`
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`casefold`、`manifest.files.values`
...
- 实现方式：
  1. 遍历 `range(0, len(candidate_bin_paths), BIN_PROBE_BATCH_SIZE)` 执行批量处理。
  2. 返回 `existing` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`extractor.extract_files`、`result.get`、`wad_result.items`、`bytes`
...
- 可能抛出：
  - RuntimeError（由语法树扫描得到）
- 关键调用：`bin_raws.items`、`BIN`、`RuntimeError`、`value.strip`、`bank_paths.append`、`_classify_voice_bank_paths`
...
- 实现方式：
  1. 遍历 `paths` 执行批量处理。
  2. 返回 `(tuple(sorted(audio.values(), key=str.casefold)), tuple(sorted(events…` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`_normalize_manifest_path`、`normalized.casefold`、`lowered.endswith`、`audio.setdefault`、`events.setdefault`、`audio.values`、`events.values`
...
- 实现方式：
  1. 遍历 `getattr(wad_header_report, 'files', tuple())` 执行批量处理。
  2. 返回 `({}, set())` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`file_path.casefold`、`region_wad_path.casefold`、`casefold`、`strip`、`path.strip`
...
- 实现方式：
  ...
  2. 根据条件 `normalized_path.casefold() in missing_paths` 执行分支处理。
  3. 调用 `set` 并写入 `candidate_hashes`。
  4. 根据条件 `old_header is not None` 执行分支处理。
  5. 根据条件 `new_header is not None` 执行分支处理。
  6. 根据条件 `not candidate_hashes` 执行分支处理。
  7. 根据条件 `not statuses` 执行分支处理。
  8. 根据条件 `'changed' in statuses` 执行分支处理。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
| `path` | `str` | `-` | `positional_or_keyword` | （Docstring 未提供） |
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`_normalize_manifest_path`、`normalized_path.casefold`、`candidate_hashes.add`、`_resolve_wad_path_hash`
...
- 实现方式：
  ...
  2. 根据条件 `callable(hash_func)` 执行分支处理。
  3. 调用 `getattr` 并写入 `fallback`。
  4. 根据条件 `callable(fallback)` 执行分支处理。
  5. 在错误场景抛出 `ValueError(f'当前 WAD 头对象不支持路径哈希计算: {type(header)!r}')`。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 可能抛出：
  - ValueError（由语法树扫描得到）
- 关键调用：`callable`、`hash_func`、`fallback`、`ValueError`
...
- 实现方式：
  ...
  2. 根据条件 `champion_match is not None` 执行分支处理。
  3. 调用 `MAP_ROOT_WAD_PATH_PATTERN.match` 并写入 `map_match`。
  4. 根据条件 `map_match is not None` 执行分支处理。
  5. 根据条件 `MAP_COMMON_ROOT_WAD_PATH_PATTERN.match(root_wad_path) is not None` 执行分支处理。
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`CHAMPION_ROOT_WAD_PATH_PATTERN.match`、`casefold`、`champion_match.group`、`MAP_ROOT_WAD_PATH_PATTERN.match`、`map_match.group`、`MAP_COMMON_ROOT_WAD_PATH_PATTERN.match`
...
- 实现方式：
  ...
  3. 根据条件 `not lowered.endswith(region_suffix.casefold())` 执行分支处理。
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`_normalize_manifest_path`、`normalized.casefold`、`lowered.endswith`、`region_suffix.casefold`
...
- 作用：标准化并去重 WAD 路径列表。
- 实现方式：
  1. 返回 `tuple(sorted({_normalize_manifest_path(path) for path in update_paths…` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`_normalize_manifest_path`、`path.strip`
...
- 实现方式：
  1. 遍历 `update_paths` 执行批量处理。
  2. 返回 `tuple((tuple(sorted(set(paths), key=str.casefold)) for _, paths in so…` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
| `update_paths` | `Sequence[str]` | `-` | `positional_or_keyword` | （Docstring 未提供） |
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`_normalize_manifest_path`、`_resolve_root_wad_path`、`normalized_path.casefold`、`casefold`、`append`、`grouped.setdefault`、`grouped.items`
...
- 实现方式：
  1. 在上下文 `WADExtractor(new_manifest, prefetch_chunk_concurr…, WADExtractor(old_manifest, prefetch_chunk_concurr…` 中执行资源操作。
...
- 返回类型：`tuple[tuple[WadVoiceFilterDecision, ...], tuple[tuple[WadVoiceFilterDecision, dict[str, b…`
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`bytes`、`bin_payloads.items`、`captured_payloads.append`、`WADExtractor`、`_build_wad_voice_filter_decision`、`decisions.append`
...
- 作用：提取 diff 条目中的路径字段。
- 实现方式：
  1. 根据条件 `isinstance(entry, dict)` 执行分支处理。
  2. 根据条件 `isinstance(value, str) and value.strip()` 执行分支处理。
  3. 在错误场景抛出 `ValueError(f'diff 条目缺少合法 path 字段：{entry!r}')`。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 可能抛出：
  - ValueError（由语法树扫描得到）
- 关键调用：`entry.get`、`value.strip`、`ValueError`
...
- 实现方式：
  ...
  2. 根据条件 `not resolved_file.exists()` 执行分支处理。
  3. 调用 `json.loads` 并写入 `payload`。
  4. 根据条件 `not isinstance(payload, dict)` 执行分支处理。
...
- 关键调用：`resolve`、`state_file.expanduser`、`resolved_file.exists`、`json.loads`、`resolved_file.read_text`、`ValueError`、`LocalRunState.from_dict`
...
- 返回说明：已写入文件路径。
- 可能抛出：未显式声明。
- 关键调用：`resolve`、`state_file.expanduser`、`resolved_file.parent.mkdir`、`resolved_file.write_text`、`json.dumps`、`state.to_dict`
...
- 实现方式：
  ...
  2. 返回 `LocalRunState(schema_version=LOCAL_STATE_SCHEMA_VERSION, game_version…` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 返回说明：本地状态对象。
- 可能抛出：未显式声明。
- 关键调用：`replace`、`isoformat`、`datetime.now`、`LocalRunState`
...
- 实现方式：
  ...
  2. 通过 `try/except` 包裹核心逻辑并处理异常。
  3. 返回 `evaluate_update_need_with_latest(region=region, latest_versions=lates…` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 返回说明：更新判定结果。
- 可能抛出：未显式声明。
- 关键调用：`get_latest_versions`、`load_local_state`、`evaluate_update_need_with_latest`
...
- 实现方式：
  ...
  3. 根据条件 `previous_state is None` 执行分支处理。
  4. 根据条件 `previous_state.game_version == latest_versions.game_version` 执行分支处理。
  ...
  8. 返回 `UpdateDecision(should_update=has_region_changes, reason=DECISION_REAS…` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
| `region` | `str` | `-` | `positional_or_keyword` | 语言区域标识（例如 `zh_CN`）。 |
...
- 返回说明：更新判定结果。
- 可能抛出：未显式声明。
- 关键调用：`ChangedEntities`、`ManifestWadChanges`、`UpdateDecision`、`get_manifest_wad_changes`、`_extract_changed_entities_from_paths`
...
- 实现方式：
  ...
  3. 遍历 `paths` 执行批量处理。
  4. 返回 `ChangedEntities(champion_aliases=tuple(sorted(champion_aliases, key=s…` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`path.replace`、`CHAMPION_WAD_PATH_PATTERN.match`、`champion_aliases.add`、`champion_match.group`、`MAP_WAD_PATH_PATTERN.match`、`map_match.group`、`map_ids.add`、`ChangedEntities`
...
- 实现方式：
  ...
  2. 根据条件 `not source_dir.is_dir()` 执行分支处理。
...
- 关键调用：`resolve`、`champion_dir.expanduser`、`source_dir.is_dir`、`FileNotFoundError`、`output_path.expanduser`、`output_dir.mkdir`、`_normalize_archive_name`、`_resolve_7zip_executable`、`_validate_compression_level`、`_normalize_extra_files`、`_normalize_report_file`、`tempfile.TemporaryDirectory`
...
- 实现方式：
  ...
  2. 根据条件 `not source_root.is_dir()` 执行分支处理。
  3. 遍历 `sorted(source_root.iterdir(), key=lambda path: path.name.casefold())` 执行批量处理。
...
- 可能抛出：
  ...
  - ValueError：参数非法时抛出。
  - RuntimeError：7z 命令执行失败时抛出。
- 关键调用：`resolve`、`audio_dir.expanduser`、`source_root.is_dir`、`FileNotFoundError`、`source_root.iterdir`、`path.name.casefold`、`item.is_dir`、`archives.append`、`pack_champion`、`_build_archive_name`、`_resolve_report_file`
...
- 作用：解析 7z 可执行文件路径。
- 实现方式：
  1. 根据条件 `seven_zip_executable` 执行分支处理。
  2. 遍历 `SEVEN_ZIP_CANDIDATES` 执行批量处理。
  3. 在错误场景抛出 `FileNotFoundError('未找到 7z 可执行文件，请安装 7-Zip 并加入 PATH')`。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 可能抛出：
  - FileNotFoundError（由语法树扫描得到）
- 关键调用：`shutil.which`、`FileNotFoundError`
...
- 作用：校验压缩级别。
- 实现方式：
  1. 根据条件 `compression_level < 0 or compression_level > MAX_COMPRESSION_LEVEL` 执行分支处理。
...
- 可能抛出：
  - FileNotFoundError（由语法树扫描得到）
- 关键调用：`resolve`、`file_path.expanduser`、`resolved_path.is_file`、`FileNotFoundError`、`resolved.append`
...
- 作用：标准化并校验报告文件。
- 实现方式：
  1. 根据条件 `report_file is None` 执行分支处理。
  2. 调用 `resolve` 并写入 `resolved`。
  3. 根据条件 `not resolved.is_file()` 执行分支处理。
  4. 返回 `resolved` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 可能抛出：
  - FileNotFoundError（由语法树扫描得到）
- 关键调用：`resolve`、`report_file.expanduser`、`resolved.is_file`、`FileNotFoundError`
...
- 作用：规范化压缩包文件名。
- 实现方式：
  1. 根据条件 `not raw_name` 执行分支处理。
  2. 根据条件 `not raw_name.endswith(ARCHIVE_SUFFIX)` 执行分支处理。
  3. 返回 `raw_name` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`archive_name.strip`、`raw_name.endswith`
...
- 作用：构建压缩包文件名。
- 实现方式：
  1. 根据条件 `version is not None and version.strip()` 执行分支处理。
  2. 根据条件 `audio_type is not None and audio_type.strip()` 执行分支处理。
...
- 作用：根据目录名解析对应报告文件。
- 实现方式：
  1. 根据条件 `report_dir is None` 执行分支处理。
  2. 调用 `resolve` 并写入 `resolved_report_dir`。
  3. 根据条件 `not resolved_report_dir.is_dir()` 执行分支处理。
  4. 调用 `strip` 并写入 `entity_id`。
  5. 根据条件 `not entity_id` 执行分支处理。
  6. 根据条件 `candidate.is_file()` 执行分支处理。
  7. 返回 `None` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`resolve`、`report_dir.expanduser`、`resolved_report_dir.is_dir`、`strip`、`folder_name.split`、`candidate.is_file`
...
- 可能抛出：
  - ValueError（由语法树扫描得到）
- 关键调用：`file_path.name.casefold`、`ValueError`、`used_names.add`、`shutil.copy2`
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`command.append`、`command.extend`
...
- 可见性：内部类
- 源码位置：`src/rift_audio_pipeline/pipeline.py:86`
- 作用：单实体打包结果。
- 装饰器：`dataclass(frozen=True, slots=True)`
- 字段：
| 字段 | 类型 | 默认值 |
...
- 可见性：内部类
- 源码位置：`src/rift_audio_pipeline/pipeline.py:94`
- 作用：压缩包上传路由信息。
- 装饰器：`dataclass(frozen=True, slots=True)`
- 字段：
| 字段 | 类型 | 默认值 |
...
- 可见性：内部类
- 源码位置：`src/rift_audio_pipeline/pipeline.py:106`
- 作用：远端索引条目解析结果。
- 装饰器：`dataclass(frozen=True, slots=True)`
- 字段：
| 字段 | 类型 | 默认值 |
...
- 可见性：公开函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:118`
- 作用：执行流水线主入口。
- 实现方式：
  1. 执行 `config.output_path.mkdir` 触发副作用逻辑。
  2. 调用 `ensure_official_sdk_path` 并写入 `sdk_dir`。
  3. 执行 `logger.info` 触发副作用逻辑。
  4. 通过 `try/except` 包裹核心逻辑并处理异常。
  5. 调用 `_normalize_pipeline_game_version` 并写入 `pipeline_game_version`。
  6. 根据条件 `not decision.should_update` 执行分支处理。
  7. 根据条件 `decision.reason == DECISION_REASON_FIRST_RUN` 执行分支处理。
  8. 根据条件 `config.dry_run` 执行分支处理。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 返回说明：退出状态码，`0` 表示执行成功。
- 可能抛出：未显式声明。
- 关键调用：`config.output_path.mkdir`、`ensure_official_sdk_path`、`logger.info`、`evaluate_update_need`、`logger.error`、`_normalize_pipeline_game_version`、`build_local_state`、`save_local_state`、`join`、`filter_wad_changes_by_bin_voice_paths`、`create_local_bin_flag`、`casefold`
...
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:576`
- 作用：判定是否启用低磁盘流式模式。
- 实现方式：
  1. 调用 `bool` 并写入 `has_targets`。
  2. 根据条件 `not (config.low_disk_mode and has_targets)` 执行分支处理。
  3. 根据条件 `runtime_is_simulated` 执行分支处理。
  4. 执行 `logger.info` 触发副作用逻辑。
  5. 返回 `False` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
...
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:594`
- 作用：解析本次运行实际生效的解包并发数。
- 实现方式：
  1. 根据条件 `runtime_is_simulated` 执行分支处理。
  2. 调用 `max` 并写入 `cpu_workers`。
  3. 调用 `max` 并写入 `effective_workers`。
  4. 根据条件 `effective_workers != configured_workers` 执行分支处理。
...
- 可能抛出：
  - RuntimeError（由语法树扫描得到）
- 关键调用：`_build_streaming_entity_tasks`、`logger.warning`、`logger.info`、`_resolve_pack_extra_files`、`_resolve_pack_archive_type`、`join`、`Queue`、`threading.Thread`、`upload_thread.start`、`RuntimeError`、`_ensure_streaming_disk_space`、`run_unpack`
...
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:754`
- 作用：构建流式模式所需的实体任务集合。
- 实现方式：
  1. 调用 `tuple` 并写入 `champion_ids`。
  2. 调用 `tuple` 并写入 `map_ids`。
  3. 遍历 `champion_ids` 执行批量处理。
  4. 遍历 `map_ids` 执行批量处理。
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`item.casefold`、`resolve_runtime_wad_paths`、`runtime_wad_index.get`、`path.casefold`、`tasks.append`、`_StreamingEntityTask`
...
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:811`
...
- 可能抛出：
  - ValueError（由语法树扫描得到）
- 关键调用：`logger.debug`、`ValueError`、`_resolve_package_output_root`、`BaiduCredentials`、`resolve_token_store`、`BaiduPanClient`、`_initialize_remote_upload_layout`、`_resolve_upload_remote_index`、`_current_utc_timestamp`、`_resolve_default_upload_resource_type`、`upload_queue.get`、`_finalize_upload_manifest`
...
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:913`
...
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:930`
...
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:945`
- 作用：在流式任务启动前执行磁盘空间检查。
- 实现方式：
...
- 可能抛出：
  - RuntimeError（由语法树扫描得到）
- 关键调用：`_estimate_streaming_required_bytes`、`_resolve_package_output_root`、`resolve`、`runtime_game_path.expanduser`、`config.output_path.expanduser`、`package_root.expanduser`、`probe.mkdir`、`check_disk_space`、`RuntimeError`
...
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:975`
- 作用：估算当前实体任务执行所需的最小可用磁盘空间。
- 实现方式：
  1. 遍历 `task.runtime_wad_paths` 执行批量处理。
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`replace`、`runtime_wad_path.strip`、`normalized.removeprefix`、`relative_path.startswith`、`Path`
...
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:1009`
- 作用：打包单实体输出并返回产物信息。
- 实现方式：
  ...
  4. 返回 `(_PackedEntityArtifact(task=task, archive_path=archive_path), entity_…` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
| `config` | `PipelineConfig` | `-` | `positional_or_keyword` | （Docstring 未提供） |
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`_resolve_entity_audio_directory`、`report_file.is_file`、`_build_entity_archive_name`、`pack_champion`、`_resolve_package_output_root`、`_PackedEntityArtifact`
...
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:1053`
- 作用：定位当前实体对应的解包输出目录。
- 实现方式：
  1. 根据条件 `not target_root.is_dir()` 执行分支处理。
  2. 调用 `tuple` 并写入 `matched_dirs`。
  3. 根据条件 `not matched_dirs` 执行分支处理。
  4. 根据条件 `len(matched_dirs) > 1` 执行分支处理。
...
- 可能抛出：
  - FileNotFoundError（由语法树扫描得到）
  - RuntimeError（由语法树扫描得到）
- 关键调用：`target_root.is_dir`、`FileNotFoundError`、`target_root.iterdir`、`item.is_dir`、`_extract_entity_id_from_directory_name`、`RuntimeError`

### 函数 `_extract_entity_id_from_directory_name`

#### `_extract_entity_id_from_directory_name(directory_name: str) -> int | None`
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:1082`
- 作用：从目录名中解析实体 ID。
- 实现方式：
  1. 调用 `strip` 并写入 `prefix`。
  2. 根据条件 `prefix.isdigit()` 执行分支处理。
  3. 返回 `None` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`strip`、`directory_name.split`、`prefix.isdigit`

### 函数 `_build_entity_archive_name`

#### `_build_entity_archive_name(directory_name: str, game_version: str, audio_type: str | None) -> str`
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:1091`
- 作用：构建单实体压缩包名称，规则与批量打包保持一致。
- 实现方式：
  1. 根据条件 `game_version.strip()` 执行分支处理。
  2. 根据条件 `isinstance(audio_type, str) and audio_type.strip()` 执行分支处理。
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`game_version.strip`、`parts.append`、`audio_type.strip`、`join`

### 函数 `_cleanup_task_runtime_wads`

#### `_cleanup_task_runtime_wads(runtime_game_path: Path, runtime_wad_paths: tuple[str, ...]) -> int`
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:1106`
- 作用：清理单实体关联的运行时 WAD 文件。
- 实现方式：
  1. 遍历 `runtime_wad_paths` 执行批量处理。
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`_resolve_runtime_wad_file`、`wad_file.is_file`、`wad_file.unlink`

### 函数 `_cleanup_entity_audio_output`

#### `_cleanup_entity_audio_output(entity_audio_dir: Path, report_file: Path | None) -> int`
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:1125`
- 作用：清理单实体解包目录与对应报告文件。
- 实现方式：
  1. 根据条件 `entity_audio_dir.is_dir()` 执行分支处理。
  2. 根据条件 `report_file is not None and report_file.is_file()` 执行分支处理。
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`entity_audio_dir.is_dir`、`entity_audio_dir.rglob`、`item.is_file`、`shutil.rmtree`、`report_file.is_file`、`report_file.unlink`

### 函数 `_cleanup_uploaded_archive`

#### `_cleanup_uploaded_archive(archive_path: Path) -> None`
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:1143`
- 作用：在上传成功后删除本地压缩包。
- 实现方式：
...
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:1149`
- 作用：解析本次运行使用的游戏目录。
- 实现方式：
  1. 根据条件 `config.game_path is not None` 执行分支处理。
...
- 可能抛出：未显式声明。
- 关键调用：`expanduser`、`resolve`、`Path`、`build_simulated_dir`、`simulated_base.resolve`、`write_content_metadata`、`logger.info`
...
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:1172`
- 作用：构建运行时下载缓存目录。
- 实现方式：
  1. 根据条件 `runtime_is_simulated` 执行分支处理。
...
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:1185`
- 作用：解析 GAME/LCU 下载目录。
- 实现方式：
  1. 根据条件 `runtime_is_simulated` 执行分支处理。
...
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:1196`
- 作用：将当前版本解包产物按目录批量打包。
- 实现方式：
  1. 根据条件 `not version_audio_dir.is_dir()` 执行分支处理。
  2. 根据条件 `config.pack_output_dir is not None` 执行分支处理。
  3. 调用 `_resolve_pack_extra_files` 并写入 `extra_files`。
  4. 调用 `_resolve_pack_archive_type` 并写入 `archive_audio_type`。
  5. 根据条件 `extra_files` 执行分支处理。
  6. 根据条件 `archive_audio_type is not None` 执行分支处理。
  7. 遍历 `pack_targets` 执行批量处理。
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`version_audio_dir.is_dir`、`logger.warning`、`_resolve_package_output_root`、`_resolve_pack_extra_files`、`_resolve_pack_archive_type`、`logger.info`、`join`、`target_dir.is_dir`、`pack_all`、`archives.extend`、`casefold`、`path.as_posix`
...
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:1253`
- 作用：在主线更新确认阶段预检远端上传索引。
- 实现方式：
  1. 根据条件 `not (config.baidu_pan_app_key and config.baidu_pan_secret_key and con…` 执行分支处理。
...
- 可能抛出：
  - ValueError（由语法树扫描得到）
- 关键调用：`ValueError`、`BaiduCredentials`、`resolve_token_store`、`BaiduPanClient`、`_initialize_remote_upload_layout`、`_load_remote_upload_manifest_index`、`logger.info`、`_clone_upload_manifest_index`、`client.close`
...
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:1294`
- 作用：将打包产物上传到百度网盘，并同步本次上传清单。
- 实现方式：
  1. 执行 `logger.debug` 触发副作用逻辑。
  2. 根据条件 `not archives` 执行分支处理。
  3. 根据条件 `not (config.baidu_pan_app_key and config.baidu_pan_secret_key and con…` 执行分支处理。
...
- 可能抛出：
  - ValueError（由语法树扫描得到）
- 关键调用：`logger.debug`、`logger.warning`、`ValueError`、`_resolve_package_output_root`、`BaiduCredentials`、`resolve_token_store`、`BaiduPanClient`、`_initialize_remote_upload_layout`、`_resolve_upload_remote_index`、`_current_utc_timestamp`、`_resolve_default_upload_resource_type`、`_process_archives_upload`
...
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:1373`
- 作用：解析本次上传使用的远端索引快照。
- 实现方式：
  1. 根据条件 `preloaded_remote_index is not None` 执行分支处理。
  2. 执行 `logger.debug` 触发副作用逻辑。
  3. 调用 `_load_remote_upload_manifest_index` 并写入 `remote_index`。
  4. 返回 `remote_index` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`_clone_upload_manifest_index`、`logger.debug`、`_load_remote_upload_manifest_index`
...
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:1395`
- 作用：复制上传索引，避免调用方状态被就地污染。
- 实现方式：
  1. 遍历 `source_index.items()` 执行批量处理。
  2. 返回 `cloned` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:1406`
- 作用：处理一批压缩包上传并更新内存索引。
- 实现方式：
  1. 遍历 `archives` 执行批量处理。
...
- 可能抛出：
  - FileNotFoundError（由语法树扫描得到）
  - RuntimeError（由语法树扫描得到）
- 关键调用：`archive.is_file`、`FileNotFoundError`、`_build_archive_upload_layout`、`_ensure_remote_directory`、`_archive_remote_old_versions`、`run_entries.extend`、`remote_index.get`、`remote_path.casefold`、`_is_same_index_entry`、`logger.info`、`run_entries.append`、`RuntimeError`
...
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:1524`
- 作用：落地本地索引文件并按需回传到远端。
- 实现方式：
  1. 调用 `_clone_upload_manifest_index` 并写入 `effective_remote_index`。
  2. 根据条件 `has_index_changes and refresh_remote_index_before_upload` 执行分支处理。
  ...
  7. 根据条件 `run_record_collector is not None` 执行分支处理。
  8. 根据条件 `has_index_changes` 执行分支处理。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
| `client` | `BaiduPanClient` | `-` | `positional_or_keyword` | （Docstring 未提供） |
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`_clone_upload_manifest_index`、`_load_remote_upload_manifest_index`、`_merge_remote_index_for_finalize`、`logger.warning`、`_build_upload_manifest_payload`、`manifest_file.parent.mkdir`、`manifest_file.write_text`、`json.dumps`、`readable_manifest_file.write_text`、`_build_readable_upload_manifest_content`、`run_record_collector.extend`、`logger.debug`
...
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:1619`
...
- 实现方式：
  ...
  3. 遍历 `removed_keys` 执行批量处理。
  4. 遍历 `upsert_keys` 执行批量处理。
  5. 返回 `merged` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`_clone_upload_manifest_index`、`_collect_remote_index_mutation_keys`、`merged.pop`、`working_remote_index.get`
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`_load_pending_manifest_sync_entries`、`logger.warning`、`logger.info`、`resolve_token_store`、`entry.get`、`resolve`、`expanduser`、`Path`、`manifest_file.is_file`、`readable_manifest_file.is_file`、`BaiduPanClient`、`BaiduCredentials`
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`logger.warning`、`_load_pending_manifest_sync_entries`、`casefold`、`remote_dir.strip`、`as_posix`、`resolve`、`manifest_file.expanduser`、`entry.get`、`remote_dir_obj.strip`、`expanduser`、`Path`、`next_entries.append`

### 函数 `_load_pending_manifest_sync_entries`

#### `_load_pending_manifest_sync_entries(queue_file: Path) -> list[dict[str, object]]`
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:1778`
- 作用：读取待重试索引回传队列。
- 实现方式：
  1. 根据条件 `not queue_file.is_file()` 执行分支处理。
  2. 通过 `try/except` 包裹核心逻辑并处理异常。
  3. 根据条件 `not isinstance(payload, list)` 执行分支处理。
  4. 遍历 `payload` 执行批量处理。
  5. 返回 `entries` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`queue_file.is_file`、`json.loads`、`queue_file.read_text`、`logger.warning`、`entries.append`
...
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:1797`
- 作用：保存待重试索引回传队列。
- 实现方式：
  1. 根据条件 `not entries` 执行分支处理。
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`queue_file.unlink`、`queue_file.parent.mkdir`、`queue_file.write_text`、`json.dumps`

### 函数 `_resolve_package_output_root`

#### `_resolve_package_output_root(config: PipelineConfig, game_version: str) -> Path`
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:1813`
- 作用：解析当前版本打包产物根目录。
- 实现方式：
  1. 根据条件 `config.pack_output_dir is not None` 执行分支处理。
...
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:1821`
- 作用：解析打包附加文件列表。
- 实现方式：
  1. 根据条件 `config.pack_extra_dir is not None` 执行分支处理。
  2. 遍历 `candidate_dirs` 执行批量处理。
  3. 返回 `tuple()` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`candidate_dirs.append`、`resolve`、`directory.expanduser`、`resolved_dir.is_dir`、`file_path.is_file`、`files.append`

### 函数 `_resolve_pack_archive_type`

#### `_resolve_pack_archive_type(config: PipelineConfig) -> str | None`
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:1844`
- 作用：解析压缩包命名的类型后缀。
- 实现方式：
  1. 调用 `sorted` 并写入 `normalized_types`。
  2. 根据条件 `len(normalized_types) == 1` 执行分支处理。
  3. 返回 `None` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`_resolve_pack_archive_type`、`upper`、`item.strip`
...
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:1874`
- 作用：解析单个压缩包的远端目录路由。
- 实现方式：
  1. 调用 `_resolve_archive_target_group` 并写入 `target_group`。
  2. 调用 `_parse_archive_file_name` 并写入 `(entity_key, _, archive_resource_type)`。
  3. 返回 `_ArchiveUploadLayout(remote_name=archive.name, remote_relative_path=r…` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:1895`
- 作用：从本地路径解析上传目标分组（champions/maps）。
- 实现方式：
  1. 遍历 `reversed(archive.parts[:-1])` 执行批量处理。
  2. 返回 `'champions'` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:1905`
- 作用：解析压缩包文件名中的实体名、版本和资源类型。
- 实现方式：
  1. 根据条件 `archive_name.casefold().endswith('.7z')` 执行分支处理。
  2. 调用 `stem.strip` 并写入 `normalized_stem`。
  3. 根据条件 `not normalized_stem` 执行分支处理。
  4. 调用 `normalized_stem.rsplit` 并写入 `parts`。
  5. 根据条件 `len(parts) != 3` 执行分支处理。
  6. 调用 `strip` 并写入 `entity_key`。
  7. 调用 `upper` 并写入 `parsed_resource_type`。
  8. 根据条件 `not entity_key` 执行分支处理。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`endswith`、`archive_name.casefold`、`stem.strip`、`normalized_stem.rsplit`、`strip`、`upper`
...
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:1930`
- 作用：将同实体旧版本远端文件归档到 `OLD/<target_group>/`。
- 实现方式：
  1. 遍历 `tuple(remote_index.items())` 执行批量处理。
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`remote_index.items`、`_resolve_index_entry_metadata`、`metadata.remote_path.casefold`、`layout.remote_path.casefold`、`_ensure_remote_directory`、`client.move_path`、`_join_remote_file_path`、`updated_entry.update`、`remote_index.pop`、`archived_remote_path.casefold`、`logger.info`、`archived_entries.append`
...
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:2004`
- 作用：解析远端索引条目的路由元数据。
- 实现方式：
  1. 调用 `entry.get` 并写入 `remote_path_value`。
  2. 根据条件 `not (isinstance(remote_path_value, str) and remote_path_value.strip())` 执行分支处理。
  3. 调用 `replace` 并写入 `remote_path`。
  4. 调用 `strip` 并写入 `remote_name`。
  5. 调用 `_build_relative_remote_path` 并写入 `raw_relative`。
  6. 调用 `_parse_archive_file_name` 并写入 `(parsed_entity_key, parsed_version, par…`。
...
- 可能抛出：
  - ValueError（由语法树扫描得到）
- 关键调用：`entry.get`、`remote_path_value.strip`、`ValueError`、`replace`、`strip`、`Path`、`_build_relative_remote_path`、`raw_relative.split`、`upper`、`casefold`、`_parse_archive_file_name`、`entry_game_version.strip`

### 函数 `_build_relative_remote_path`

#### `_build_relative_remote_path(remote_path: str, remote_dir: str) -> str`
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:2059`
- 作用：将绝对远端路径转换为工作目录下相对路径。
- 实现方式：
  1. 调用 `replace` 并写入 `normalized_remote_path`。
  2. 根据条件 `not normalized_remote_path.startswith('/')` 执行分支处理。
  3. 调用 `strip` 并写入 `normalized_dir`。
  4. 根据条件 `normalized_work_dir == '/'` 执行分支处理。
  5. 根据条件 `normalized_remote_path.casefold().startswith(f'{normalized_work_dir}/…` 执行分支处理。
  6. 根据条件 `normalized_remote_path.casefold() == normalized_work_dir.casefold()` 执行分支处理。
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`replace`、`remote_path.strip`、`normalized_remote_path.startswith`、`normalized_remote_path.lstrip`、`strip`、`remote_dir.strip`、`startswith`、`normalized_remote_path.casefold`、`casefold`、`normalized_work_dir.casefold`

### 函数 `_ensure_remote_directory`

#### `_ensure_remote_directory(client: BaiduPanClient, relative_dir: str) -> None`
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:2076`
- 作用：确保远端目录存在（按层创建）。
- 实现方式：
  1. 调用 `strip` 并写入 `normalized`。
  2. 根据条件 `not normalized` 执行分支处理。
  3. 遍历 `normalized.split('/')` 执行批量处理。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
| `client` | `BaiduPanClient` | `-` | `positional_or_keyword` | （Docstring 未提供） |
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`strip`、`replace`、`relative_dir.strip`、`normalized.split`、`_remote_file_exists`、`client.create_directory`

### 函数 `_initialize_remote_upload_layout`

#### `_initialize_remote_upload_layout(client: BaiduPanClient) -> None`
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:2091`
- 作用：初始化远端目录结构。
- 实现方式：
  1. 执行 `client.create_directory` 触发副作用逻辑。
  2. 遍历 `RESOURCE_TYPE_BUCKETS` 执行批量处理。
  3. 遍历 `RESOURCE_TARGET_GROUPS` 执行批量处理。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
| `client` | `BaiduPanClient` | `-` | `positional_or_keyword` | （Docstring 未提供） |
...
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:2104`
- 作用：构建给人类阅读和搜索的上传索引文本。
- 实现方式：
  1. 遍历 `sorted(index.values(), key=lambda item: str(item.get('remote_path', '…` 执行批量处理。
  2. 根据条件 `active_lines` 执行分支处理。
  3. 执行 `lines.extend` 触发副作用逻辑。
  4. 根据条件 `archived_lines` 执行分支处理。
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`index.values`、`casefold`、`item.get`、`_resolve_index_entry_metadata`、`archived_lines.append`、`active_lines.append`、`lines.extend`、`lines.append`、`join`
...
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:2161`
- 作用：将远端索引聚合为实体维度数据库视图。
- 实现方式：
  1. 遍历 `sorted(index.values(), key=lambda item: str(item.get('remote_path', '…` 执行批量处理。
  2. 遍历 `grouped.items()` 执行批量处理。
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`index.values`、`casefold`、`item.get`、`_resolve_index_entry_metadata`、`grouped.get`、`versions.append`、`grouped.items`、`value.get`、`_parse_game_version_sort_key`、`reversed`、`latest_version.get`

### 函数 `_parse_game_version_sort_key`

#### `_parse_game_version_sort_key(game_version: str, fallback: str) -> tuple[int, ...]`
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:2230`
- 作用：将版本号解析为可排序键。
- 实现方式：
  ...
  4. 遍历 `parts[:4]` 执行批量处理。
  5. 在条件 `len(parsed) < 4` 成立时循环处理。
  6. 根据条件 `all((value == -1 for value in parsed))` 执行分支处理。
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`game_version.strip`、`normalized.split`、`part.isdigit`、`parsed.append`、`ord`、`fallback.casefold`
...
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:2251`
...
- 实现方式：
  ...
  7. 根据条件 `not (config.baidu_pan_app_key and config.baidu_pan_secret_key and con…` 执行分支处理。
...
- 可能抛出：
  - ValueError（由语法树扫描得到）
- 关键调用：`_current_utc_timestamp`、`strip`、`replace`、`executed_at.replace`、`report_dir.mkdir`、`_build_update_log_payload`、`json_file.write_text`、`json.dumps`、`text_file.write_text`、`_build_update_log_text`、`ValueError`、`BaiduCredentials`
...
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:2319`
- 作用：构建单次 diff 的结构化更新日志。
- 实现方式：
  1. 调用 `getattr` 并写入 `wad_changes_obj`。
  2. 根据条件 `secondary_filter_result is not None` 执行分支处理。
  ...
  8. 根据条件 `secondary_filter is not None` 执行分支处理。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
| `decision` | `object` | `-` | `positional_or_keyword` | （Docstring 未提供） |
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`filter_decisions.append`、`item.get`

### 函数 `_build_update_log_text`

#### `_build_update_log_text(payload: dict[str, object]) -> str`
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:2406`
...
- 实现方式：
  ...
  5. 根据条件 `isinstance(secondary_filter_obj, dict)` 执行分支处理。
  6. 执行 `lines.extend` 触发副作用逻辑。
  7. 调用 `upload_summary.get` 并写入 `entries_obj`。
  8. 根据条件 `isinstance(entries_obj, list)` 执行分支处理。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`payload.get`、`join`、`changed_entities.get`、`wad_changes.get`、`lines.extend`、`secondary_filter_obj.get`、`lines.append`、`item.get`、`upload_summary.get`

### 函数 `_calculate_sha256`

#### `_calculate_sha256(file_path: Path) -> str`
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:2480`
- 作用：计算文件 SHA256。
- 实现方式：
  1. 调用 `hashlib.sha256` 并写入 `digest`。
  2. 在上下文 `file_path.open('rb')` 中执行资源操作。
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`hashlib.sha256`、`file_path.open`、`stream.read`、`digest.update`、`digest.hexdigest`
...
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:2490`
- 作用：读取远端上传索引并构建内存查询表。
- 实现方式：
  1. 通过 `try/except` 包裹核心逻辑并处理异常。
- 参数：
...
- 关键调用：`client.download_file`、`RuntimeError`、`logger.info`、`json.loads`、`temp_file.read_text`、`ValueError`、`payload.get`、`_build_upload_manifest_index`、`temp_file.unlink`

### 函数 `_build_upload_manifest_index`

#### `_build_upload_manifest_index(entries: object) -> dict[str, dict[str, object]]`
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:2547`
- 作用：将远端索引条目转换为高效查询结构。
- 实现方式：
  1. 根据条件 `not isinstance(entries, list)` 执行分支处理。
  2. 遍历 `entries` 执行批量处理。
  3. 返回 `index` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
...
- 可能抛出：
  - ValueError（由语法树扫描得到）
- 关键调用：`ValueError`、`raw_entry.get`、`remote_path_obj.strip`、`replace`、`strip`、`Path`、`field_value.strip`、`_coerce_non_negative_int`、`sha256_obj.strip`、`lower`、`remote_path.casefold`
...
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:2589`
- 作用：构建上传索引快照。
- 实现方式：
  ...
  4. 调用 `sum` 并写入 `skipped_count`。
  5. 调用 `sum` 并写入 `archived_count`。
  6. 返回 `{'schema_version': UPLOAD_MANIFEST_SCHEMA_VERSION, 'updated_at': exec…` 作为结果。
- 参数：
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
| `game_version` | `str` | `-` | `positional_or_keyword` | （Docstring 未提供） |
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`index.values`、`casefold`、`entry.get`、`_build_upload_database_payload`

### 函数 `_join_remote_file_path`

#### `_join_remote_file_path(remote_dir: str, remote_name: str) -> str`
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:2628`
- 作用：拼接远端目录和文件名。
- 实现方式：
  1. 调用 `lstrip` 并写入 `normalized_name`。
  2. 根据条件 `not normalized_name` 执行分支处理。
  3. 调用 `strip` 并写入 `normalized_dir`。
  4. 根据条件 `not normalized_dir` 执行分支处理。
...
- 可能抛出：
  - ValueError（由语法树扫描得到）
- 关键调用：`lstrip`、`replace`、`remote_name.strip`、`ValueError`、`strip`、`remote_dir.strip`
...
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:2641`
- 作用：按文件名与版本号判断索引条目是否匹配。
- 实现方式：
  1. 调用 `entry.get` 并写入 `indexed_name`。
  2. 根据条件 `isinstance(indexed_name, str) and indexed_name.strip()` 执行分支处理。
  3. 调用 `entry.get` 并写入 `indexed_version`。
  4. 根据条件 `indexed_version is None` 执行分支处理。
  5. 根据条件 `not isinstance(indexed_version, str)` 执行分支处理。
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`entry.get`、`indexed_name.strip`、`indexed_version.strip`

### 函数 `_remote_file_exists`

#### `_remote_file_exists(client: BaiduPanClient, remote_path: str) -> bool`
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:2661`
- 作用：判断远端路径是否已存在。
- 实现方式：
  1. 通过 `try/except` 包裹核心逻辑并处理异常。
- 参数：
...
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:2671`
- 作用：返回 UTC ISO-8601 时间戳。
- 实现方式：
  1. 返回 `datetime.now(tz=timezone.utc).isoformat(timespec='seconds').replace('…` 作为结果。
- 参数：
参数：无。
- 返回类型：`str`
- 返回说明：（Docstring 未提供）
...
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:2677`
- 作用：将对象转换为非负整数。
- 实现方式：
  1. 根据条件 `isinstance(value, bool)` 执行分支处理。
  2. 根据条件 `isinstance(value, int)` 执行分支处理。
...
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:2696`
- 作用：清理模拟目录下临时 WAD 与下载缓存。
- 实现方式：
  1. 遍历 `runtime_wad_dirs` 执行批量处理。
  2. 根据条件 `runtime_download_dir.expanduser().resolve() == runtime_game_path.expa…` 执行分支处理。
  3. 根据条件 `runtime_download_dir.exists()` 执行分支处理。
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`directory.is_dir`、`directory.rglob`、`item.is_file`、`endswith`、`item.name.casefold`、`item.unlink`、`resolve`、`runtime_download_dir.expanduser`、`runtime_game_path.expanduser`、`runtime_download_dir.exists`、`runtime_download_dir.rglob`、`shutil.rmtree`

### 函数 `_merge_runtime_wad_paths`

#### `_merge_runtime_wad_paths(*groups: tuple[str, ...]) -> tuple[str, ...]`
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:2727`
...
| 参数 | 类型 | 默认值 | 参数类别 | 说明 |
| --- | --- | --- | --- | --- |
| `*groups` | `tuple[str, ...]` | `-` | `var_positional` | 可变位置参数。 |
- 返回类型：`tuple[str, ...]`
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`replace`、`path.strip`、`deduped.setdefault`、`normalized.casefold`、`deduped.values`

### 函数 `_normalize_pipeline_game_version`

#### `_normalize_pipeline_game_version(game_version: str) -> str`
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:2740`
- 作用：将版本号标准化为 `major.minor`，用于本地路径与打包命名。
- 实现方式：
  1. 调用 `game_version.strip` 并写入 `normalized`。
  2. 根据条件 `not normalized` 执行分支处理。
  3. 调用 `normalized.split` 并写入 `parts`。
  4. 根据条件 `len(parts) >= 2 and parts[0].isdigit() and parts[1].isdigit()` 执行分支处理。
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`game_version.strip`、`normalized.split`、`isdigit`
...
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:2752`
- 作用：将 manifest WAD 路径转换为运行时根/区域路径集合。
- 实现方式：
  1. 遍历 `manifest_paths` 执行批量处理。
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`replace`、`raw_path.strip`、`normalized.startswith`、`normalized.removeprefix`、`runtime_paths.setdefault`、`region_runtime_path.casefold`、`normalized.casefold`、`lowered.endswith`、`region_suffix.casefold`、`root_runtime_path.casefold`、`runtime_paths.values`

### 函数 `_cleanup_version_audio_outputs`

#### `_cleanup_version_audio_outputs(version_audio_dir: Path) -> int`
- 可见性：内部函数
- 源码位置：`src/rift_audio_pipeline/pipeline.py:2782`
- 作用：清理已打包版本的音频目录，降低上传阶段磁盘占用。
- 实现方式：
  1. 根据条件 `not version_audio_dir.is_dir()` 执行分支处理。
  2. 遍历 `version_audio_dir.rglob('*')` 执行批量处理。
...
- 返回说明：（Docstring 未提供）
- 可能抛出：未显式声明。
- 关键调用：`version_audio_dir.is_dir`、`version_audio_dir.rglob`、`item.is_file`、`shutil.rmtree`
